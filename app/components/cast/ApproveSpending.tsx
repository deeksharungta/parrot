...
interface ApproveSpendingProps {
  onClose: () => void;
  isOpen: boolean;
}

export function ApproveSpending({ onClose, isOpen }: ApproveSpendingProps) {
  return (
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            ...
            <p className="text-sm text-[#8C8A94]">
              to cast tweets, we need you to approve USDC spending
            </p>
            ...
            <div className="flex flex-col gap-1 bg-[#FFF4E6] rounded-xl p-3">
              ...
              <p className="text-sm font-light text-[#FF8C00]">
                You need to approve USDC spending to cast tweets. This sets a
                spending limit and ensures secure payments for your casts.
              </p>
            </div>
            <Link href="/approval">
              <Button className="mt-6">Approve USDC Spending</Button>
            </Link>
          </motion.div>
        </>
      )}
